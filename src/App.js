...
export default function App() {
    ...
    const halfElapsed = useMemo(() => state.currentHalf === 1
        ? gameSeconds
        : Math.max(0, gameSeconds - HALF_SECONDS), [gameSeconds, state.currentHalf]);
    ...
    const topScorersByTeam = useMemo(() => state.teams.map((t) => {
        const rows = Object.entries(t.byPlayer).map(([num, r]) => ({
            number: Number(num),
            name: Number(num) === 0
                ? "Unknown"
