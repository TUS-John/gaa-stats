...
function withHistory(state: AppState, next: AppState): AppState {
  const cap = (state.history || []).slice(-49);
  return { ...next, history: [...cap, clone(state)] };
}
...
export default function reducer(state: AppState, action: any): AppState {
  switch (action.type) {
    ...
    case "SETUP": {
      ...
      next.teams[0].positionByNumber = buildPositionByNumber(
        next.teams[0].onField
      );
      next.teams[1].name = teamB || "Team B";
      next.teams[1].squad = defaultSquad(rosterB || []);
      next.teams[1].onField = defaultOnField();
      next.teams[1].positionByNumber = buildPositionByNumber(
        next.teams[1].onField
      );
      return next;
    }
    case "TICK": {
    ...
    case "PAUSE": {
      ...
      const elapsed =
        (state.elapsedMs || 0) +
        (action.nowMs - (state.runAnchorMs || action.nowMs));
      return {
        ...state,
        running: false,
        elapsedMs: elapsed,
    ...
    case "NEXT_HALF": {
      ...
      const currentMs =
        (state.elapsedMs || 0) +
        (state.running && state.runAnchorMs ? nowMs - state.runAnchorMs : 0);
      const newElapsed =
      ...
      return {
        ...
        runAnchorMs: state.running ? nowMs : null,
      };
    }
    case "RESET_TIME": {
      ...
      for (const k in state.yellows) {
        ...
        ny[k] = recs.map((r: any) => ({
          ...r,
          at: 0,
          expiresAt: Math.max(0, (r.expiresAt || 0) - now),
        }));
      }
      next.yellows = ny;
    ...
    case "RESET_ALL": {
      ...
      next.yellows = {};
      next.reds = {};
      for (const t of next.teams as any[]) {
    ...
    case "SCORE": {
      ...
      team.byPlayer[n] = team.byPlayer[n] || {
        name: n === 0 ? "Unknown" : squadEntry.name || `#${n}`,
        goals: 0,
        points: 0,
        freesGoals: 0,
        freesPoints: 0,
      ...
      if (kind === "goal") {
        ...
        if (via === "free") team.byPlayer[n].freesGoals += 1;
      } else {
        team.byPlayer[n].points += 1;
        if (via === "free") team.byPlayer[n].freesPoints += 1;
      }
      next.events.push({
        t: action.nowSec,
        ...
        type:
          kind === "goal"
            ...
            : "Point",
        playerNumber: n,
        name: n === 0 ? "Unknown" : squadEntry.name || "",
      });
      return withHistory(state, next);
    }
    case "CARD": {
      ...
      const hasActive =
        Array.isArray((next.yellows as any)[k]) &&
        (next.yellows as any)[k].some((r: any) => r.expiresAt > action.nowSec);
      if (hasActive) {
        (next.reds as any)[k] = true;
        delete (next.yellows as any)[k];
        if (pos != null) {
          team.onField[pos] = null;
          delete team.positionByNumber[n];
        }
      ...
      (next.yellows as any)[k] = [{ at: action.nowSec, expiresAt }];
      if (pos != null) {
        team.onField[pos] = null;
        delete team.positionByNumber[n];
      }
    ...
    case "RESTORE_SINBIN": {
      ...
      const recs = (next.yellows as any)[k];
      if (!Array.isArray(recs) || recs.length === 0) return state;
      // Find first empty slot (ignore saved position)
    ...
    case "UNDO": {
      ...
      return { ...prev, history: hist.slice(0, -1) } as AppState;
    }
    case "SET_HALF":
      return { ...state, currentHalf: action.half };
    case "IMPORT":
      return action.state;
    default:
      return state;
