...
const defaultSquad = (names = []) => Array.from({ length: 30 }, (_, i) => ({ number: i + 1, name: names[i] || '' }));
const defaultOnField = () => Array.from({ length: 15 }, (_, i) => i + 1);
const buildPositionByNumber = (on) => { const m = {}; on.forEach((num, pos) => { if (num != null)
    m[num] = pos; }); return m; };
export const key = (teamIdx, number) => `${teamIdx}|${number}`;
function withHistory(state, next) { const cap = (state.history || []).slice(-49); return { ...next, history: [...cap, clone(state)] }; }
const initialTeam = () => ({ name: 'Team', squad: defaultSquad([]), onField: defaultOnField(), positionByNumber: buildPositionByNumber(defaultOnField()), score: { goals: 0, points: 0 }, byPlayer: {} });
export const initialState = { setupComplete: false, teams: [{ ...initialTeam(), name: 'Home' }, { ...initialTeam(), name: 'Away' }], running: false, elapsedMs: 0, runAnchorMs: null, heartbeat: 0, currentHalf: 1, events: [], yellows: {}, reds: {}, history: [] };
export default function reducer(state, action) {
    switch (action.type) {
        ...
        case 'TICK': {
            if (!state.running)
                return state;
            return { ...state, heartbeat: (state.heartbeat || 0) + 1 };
        }
        case 'START': {
            if (state.running)
                return state;
            return { ...state, running: true, runAnchorMs: action.nowMs };
        }
        case 'PAUSE': {
            if (!state.running)
                return state;
            const elapsed = (state.elapsedMs || 0) + (action.nowMs - (state.runAnchorMs || action.nowMs));
            return { ...state, running: false, elapsedMs: elapsed, runAnchorMs: null };
        }
        case 'NEXT_HALF': {
            const nowMs = action.nowMs;
            const currentMs = (state.elapsedMs || 0) + (state.running && state.runAnchorMs ? (nowMs - state.runAnchorMs) : 0);
            const newElapsed = currentMs < HALF_SECONDS * 1000 ? HALF_SECONDS * 1000 : currentMs;
            return { ...state, currentHalf: 2, elapsedMs: newElapsed, runAnchorMs: state.running ? nowMs : null };
        }
        case 'RESET_TIME': {
            const now = action.nowSec || 0;
            const next = clone(state);
            next.running = false;
            next.elapsedMs = 0;
            next.runAnchorMs = null;
            next.currentHalf = 1;
            const ny = {};
            for (const k in state.yellows) {
                ...
                if (!Array.isArray(recs))
                    continue;
                ny[k] = recs.map((r) => ({ ...r, at: 0, expiresAt: Math.max(0, (r.expiresAt || 0) - now) }));
            }
            next.yellows = ny;
        ...
        case 'SCORE': {
            ...
            if (kind === 'goal') {
                team.byPlayer[n].goals += 1;
                if (via === 'free')
                    team.byPlayer[n].freesGoals += 1;
            }
            else {
                team.byPlayer[n].points += 1;
                if (via === 'free')
                    team.byPlayer[n].freesPoints += 1;
            }
        ...
        case 'CARD': {
            ...
            const pos = team.positionByNumber[n];
            if (card === 'red') {
                next.reds[k] = true;
                if (pos != null) {
                ...
                next.events.push({ t: action.nowSec, half: next.currentHalf, teamIdx, type: 'Red Card', playerNumber: n, name: (n === 0 ? 'Unknown' : (team.squad[n - 1]?.name || '')) });
                return withHistory(state, next);
            }
            const hasActive = Array.isArray(next.yellows[k]) && next.yellows[k].some((r) => r.expiresAt > action.nowSec);
            if (hasActive) {
                next.reds[k] = true;
                delete next.yellows[k];
                if (pos != null) {
                    team.onField[pos] = null;
                    delete team.positionByNumber[n];
                }
                next.events.push({ t: action.nowSec, half: next.currentHalf, teamIdx, type: 'Second Yellow → Red', playerNumber: n, name: (n === 0 ? 'Unknown' : (team.squad[n - 1]?.name || '')) });
                return withHistory(state, next);
            }
            const expiresAt = Math.min(action.nowSec + 10 * 60, TOTAL_GAME_SECONDS);
            next.yellows[k] = [{ at: action.nowSec, expiresAt, pos }];
            if (pos != null) {
                team.onField[pos] = null;
                delete team.positionByNumber[n];
            }
        ...
        case 'MOVE': {
            ...
            const next = clone(state);
            const team = next.teams[teamIdx];
            const srcPos = team.positionByNumber[sourceNumber];
            if (srcPos == null)
                return state;
            if (FRONT_THREE.includes(srcPos) && FRONT_THREE.includes(targetPos)) {
                const cycle = FRONT_THREE;
                const tIdx = cycle.indexOf(targetPos);
                if (tIdx !== -1) {
                    const path = [];
                    for (let i = 0; i < cycle.length; i++)
                        path.push(cycle[(tIdx + i) % cycle.length]);
                    const nums = path.map((p) => team.onField[p]);
                    const srcNum = team.onField[srcPos];
                    const after = [srcNum, ...nums.slice(0, nums.length - 1)];
                    path.forEach((p, i) => { team.onField[p] = after[i]; if (after[i] != null)
                        team.positionByNumber[after[i]] = p; });
            ...
            team.onField[targetPos] = action.sourceNumber;
            team.positionByNumber[action.sourceNumber] = targetPos;
            if (targetOccupant != null) {
                team.onField[srcPos] = targetOccupant;
                team.positionByNumber[targetOccupant] = srcPos;
            }
            else {
                team.onField[srcPos] = null;
                delete team.positionByNumber[action.sourceNumber];
            }
        ...
        case 'RESTORE_SINBIN': {
            ...
            const recs = next.yellows[k];
            if (!Array.isArray(recs) || recs.length === 0)
                return state;
            const rec = recs[0];
            if (team.positionByNumber[number] != null) {
                delete next.yellows[k];
                return withHistory(state, next);
            }
            let targetPos = (rec.pos != null && team.onField[rec.pos] == null) ? rec.pos : null;
            if (targetPos == null) {
                for (let i = 0; i < 15; i++) {
        ...
        case 'UNDO': {
            ...
            return { ...prev, history: hist.slice(0, -1) };
        }
